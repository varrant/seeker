<template>
<div class="container">
  <md-layout md-gutter v-for="repo in list" v-bind:data="repo" v-bind:key="repo">
  ...
  <input type="button" @click="getRepository" value="测试"></input>
</div>
</template>
...
export default {
  ...
  methods: {
    getRepository() {
      ...
      this.axios.post(this.$store.state.api.pesonals, params)
        .then(response => {
          console.info(response);
          this.list = response.data;
        });
      this.axios.get(this.$store.state.api.head_details + '?task_id=1')
        .then(response => {
          console.info(response);
          this.list = response.data;
        });
